...
- 限速固定 3.5 秒一条（Telegram 频道限制约 20 条/分钟）——这是硬下限，不建议调小：这台机器上网络代理偶发抽风，跟限速的安全余量一起吃掉才扛得住重试；一部 2000+ 分段的视频大概两小时上下
- D1 批量写（默认每 100 段一批）放在后台线程做，不卡限速主循环——`wrangler d1 execute` 进程冷启动 + API 往返这部分开销藏在本来就要等的 3.5 秒里，实测能省掉真实总耗时里约三成的 D1 相关开销
